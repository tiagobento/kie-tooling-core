...
import { EditorApi, KogitoEditorChannelApi, KogitoEditorEnvelopeContextType } from "@kie-tooling-core/editor/dist/api";
...
<svg version="1.1" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" 
  ...
  const tweakContrast = useCallback(
    (value: string) => {
      ...
      stateControl.updateCommandStack({ id: JSON.stringify(command.id) });
